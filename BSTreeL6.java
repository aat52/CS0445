...
class BSTreeL6<T>
{
  ...
  private void calcLevelCounts( BSTNode root, int levelCounts[], int level )
  {
	...
	public BSTreeL6<T> makeBalancedCopyOf( )
	{
        ...
        addKeys(keys, 0, countNodes() - 1, balancedBST);
    ...
    void addKeys(ArrayList<T> keys, int low, int high, BSTreeL6<T> balancedBST){
        int mid = (high + low)/2;
        balancedBST.add(keys.get(mid));
        if (low > high)
            return;
        addKeys(keys, low, mid - 1, balancedBST);
        addKeys(keys, mid + 1, high, balancedBST);        
    }
